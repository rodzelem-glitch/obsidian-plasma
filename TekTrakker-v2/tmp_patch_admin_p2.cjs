...
// The above might break message: content and caption: content that have commas after them in the same line! 
// Let's reset those back manually.
...
c = c.replace(/gbFn\(\{ content: baseContent, accessToken:/, "gbFn({ content: platformContent.gb || baseContent, accessToken:");
...
// Also fallback if the previous regex triggered
...
// Fix OmniPreviewBoard binding at bottom (also might be baseContent now)
