...
import { Send, X, Loader2, Maximize, Minimize, Image as ImageIcon, Wrench, Cpu, ArrowLeft, CheckCircle2, ChevronRight, FileText, Bot } from 'lucide-react';
...
export async function executeSynthesizedTool(orgId: string, params: any) {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const askVirtualWorker = httpsCallable(functions, 'aiAgentController', { timeout: 540000 });

            let imagePayload = null;
            let downloadUrl: string | undefined = undefined;

            if (imageFile) {
                const path = `ai_chats/${organizationId || 'general'}/${jobId || 'general'}/${Date.now()}_${imageFile.name}`;
                ...
                // Attempt cloud storage upload if online
                if (typeof navigator !== 'undefined' && navigator.onLine) {
                    try {
                        downloadUrl = await uploadFileToStorage(path, imageFile);
                    } catch (storageErr) {
                        console.warn("[SmartTechAssistant] Direct storage upload failed, queueing offline:", storageErr);
                    }
                }

                // If offline or storage upload was deferred, stage base64 dataUrl and queue
                if (!downloadUrl) {
                    downloadUrl = `data:${imageFile.type || 'image/jpeg'};base64,${base64Image}`;
            ...
            if (jobId && organizationId) {
                ...
                try {
                    ...
                    console.warn("[SmartTechAssistant] Local offline message staging for Firestore:", fsErr);
                }
            }

            let response: any;
            if (typeof navigator !== 'undefined' && !navigator.onLine) {
                const localReply = "You are currently offline. Your question and photo have been recorded locally and will be synced once connection is restored. Standard equipment specs and job notes remain accessible offline.";
                response = { data: { reply: localReply } };
            } else {
                // Call Paid AI Virtual Worker backend
                response = await askVirtualWorker({
                    prompt: userMessageText,
                    imageBase64: imagePayload?.data || null,
                    imageMimeType: imagePayload?.mimeType || null,
                    history: messages.slice(-10).map(m => ({ role: m.sender === 'user' ? 'user' : 'model', parts: [{ text: m.text || '' }] })),
                    jobId: jobId || null,
                    organizationId: organizationId || null,
                    currentPath: window.location.pathname
                }).catch(async () => {
                    // Fallback to callGeminiAI if askVirtualWorker is not configured
                    const callGeminiAI = httpsCallable(functions, 'callGeminiAI');
                    return await callGeminiAI({
                        prompt: userMessageText,
                        image: imagePayload,
                        systemInstruction: `You are the AI Virtual Worker for field operations and technical assistance.`
                    });
                });
            }

            const replyText = response.data.reply || response.data.response || response.data.text || response.data.content || "Command processed.";
            ...
            console.error("AI Virtual Worker Error:", err);
            setError(err.message || "Failed to reach AI Virtual Worker. Please try again.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="AI Virtual Worker" size={isMaximized ? "full" : "md"}>
            <div className={`flex flex-col ${isMaximized ? 'h-full lg:h-[calc(100vh-80px)]' : 'h-[80dvh] max-h-[600px] min-h-[360px]'} bg-gray-50 dark:bg-slate-800`}>
                
                {/* Modern Header with AI Virtual Worker Branding */}
                <div className="flex flex-col border-b border-slate-200 dark:border-slate-800 bg-slate-900 text-white px-4 pt-4 shadow-sm">
                    <div className="flex justify-between items-center pb-2">
                        <div className="flex items-center gap-2.5">
                            <div className="w-9 h-9 bg-blue-600 rounded-full flex items-center justify-center border border-blue-400 shadow-md">
                                <Bot className="w-5 h-5 text-white" />
                            </div>
                            <div>
                                <h3 className="text-base font-black text-white uppercase tracking-wider flex items-center gap-2">
                                    AI Virtual Worker
                                    <span className="text-[10px] font-extrabold bg-emerald-500 text-white px-2 py-0.5 rounded-full uppercase tracking-normal">Paid AI Worker</span>
                                </h3>
                                <p className="text-[11px] text-slate-300">Automated Technical & Field Operations Assistant</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-1">
                            <Button variant="ghost" size="sm" onClick={() => setIsMaximized(!isMaximized)} className="h-8 w-8 p-0 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800">
                                {isMaximized ? <Minimize size={18} /> : <Maximize size={18} />}
                            </Button>
                            <Button variant="ghost" size="sm" onClick={onClose} className="h-8 w-8 p-0 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800">
                                <X size={18} />
                            </Button>
                        </div>
                    </div>
