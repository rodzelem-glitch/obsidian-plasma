            ...
            <Card>
                ...
                {assets.length > 0 ? assets.map(a => (
                    <div key={a.id} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg mb-3 border border-slate-200 dark:border-slate-700 flex justify-between items-start">
                        <div>
                            <p className="font-bold text-sm text-slate-800 dark:text-slate-100">{a.name ? `${a.name} (${a.brand} ${t(a.type)})` : `${a.brand} ${t(a.type)}`}</p>
                            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">{t("Serial Number")}: {a.serial}</p>
                            <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">{t("Location")}: {a.location || 'N/A'}</p>
                        </div>
                ...
                {isAddAssetOpen && (
                    <div className="mt-4 p-4 border border-primary-200 bg-primary-50/30 dark:bg-slate-800 rounded-xl space-y-4">
                        <h5 className="font-bold text-slate-800 dark:text-slate-100">{newAsset.id ? t("Edit Asset") : t("Add New Asset")}</h5>
                        <div className="flex flex-col gap-1">
                            ...
                            <Select label={t("Type")} value={newAsset.type || 'System'} onChange={e => setNewAsset({...newAsset, type: e.target.value})}>
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <Input label={t("Sub-Location (e.g. Attic)")} value={newAsset.location || ''} onChange={e => setNewAsset({...newAsset, location: e.target.value})} placeholder="e.g. Roof, Basement"/>
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700 mt-2">
                            ...
                            <Button onClick={handleAddAsset} disabled={isOcrScanning}>{isOcrScanning ? t("Scanning...") : t("Save Asset")}</Button>
                        </div>
                    </div>
                )}
            </Card>
