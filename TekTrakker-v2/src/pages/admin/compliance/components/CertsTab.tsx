                        ...
                        <Card key={emp.id} className={`border-l-4 ${isCompliant ? 'border-green-500' : 'border-orange-500'}`}>
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{t(emp.role)}</p>
                                </div>
                                {isCompliant ? <CheckCircle className="text-green-500" size={20}/> : <AlertTriangle className="text-orange-500" size={20}/>}
                            </div>
                            
                            <div className="space-y-2 mb-4">
                                <p className="text-xs font-bold text-gray-500 uppercase">{t("Uploaded Certs")}</p>
                                ...
                                ) : <p className="text-xs text-gray-400 italic">{t("None uploaded")}</p>}
                            </div>

                            {!isCompliant && requiredCerts.length > 0 && (
                                <div className="mb-4">
                                    <p className="text-xs font-bold text-red-500 uppercase">{t("Missing Requirements")}</p>
                                    <ul className="text-xs text-red-400 list-disc list-inside">
                                        {missingCerts.map(c => <li key={c}>{t(c)}</li>)}
                                    </ul>
                                </div>
                            )}
