...
const CustomerMasterModal: React.FC<CustomerMasterModalProps> = ({ isOpen, onClose, customerId }) => {
    ...
    const handleSaveOverview = async () => {
        ...
        // Ensure we have an organization ID. If the customer record is missing it,
        // we default to the current user's organization. This fixes permissions errors
        // for orphaned records.
        const organizationId = customer.organizationId || state.currentOrganization?.id;

        if (!organizationId) {
            alert("Error: No organization context found. Cannot save customer.");
            return;
        }

        const updates = {
            ...formData,
            organizationId // Ensure this is present for security rules check
        };

        try {
            await db.collection('customers').doc(customer.id).update(updates);
            
            dispatch({ type: 'UPDATE_CUSTOMER', payload: { ...customer, ...updates } });
            setIsEditing(false);
        } catch (error) {
            console.error("Failed to save customer overview:", error);
            alert("An error occurred while saving. Please ensure you have permission to edit this record.");
        }
    };
                ...
                <div className="h-[50vh] overflow-y-auto custom-scrollbar p-1">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 sm:grid-cols-3 gap-3">
                                        {state.membershipPlans.map(plan => (
                                            <button 
                                                ...
                                                className="p-3 bg-white dark:bg-gray-800 border-2 border-purple-100 dark:border-purple-800 rounded-lg hover:border-purple-500 text-left transition-all"
                                            >
                    ...
                    {activeTab === 'docs' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {allFiles.map(file => (
                                <div key={file.id} className="relative group bg-gray-100 dark:bg-gray-700 rounded h-32 overflow-hidden border border-gray-200 dark:border-gray-700">
                                    {file.fileType.includes('image') ? (
                                        <img src={file.dataUrl} alt={file.fileName} className="w-full h-full object-cover" />
                                    ) : (
                                        <div className="w-full h-full flex items-center justify-center text-xs text-gray-500">{file.fileName}</div>
                                    )}
