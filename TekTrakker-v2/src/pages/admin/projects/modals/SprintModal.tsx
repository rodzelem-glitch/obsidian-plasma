...
const SprintModal: React.FC<SprintModalProps> = ({ isOpen, onClose, onSave, sprintForm, setSprintForm }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={sprintForm.id ? t('Edit Sprint') : t('Create Sprint')}>
            <div className="space-y-4">
                <Input 
                    label={t("Sprint Name")} 
                    value={sprintForm.name || ''} 
                    onChange={e => setSprintForm({...sprintForm, name: e.target.value})} 
                    placeholder={t("Sprint 1, Week of 4/28, etc.")} 
                />

                <Input 
                    label={t("Sprint Goal (what are we trying to accomplish?)")} 
                    value={sprintForm.goal || ''} 
                    onChange={e => setSprintForm({...sprintForm, goal: e.target.value})} 
                    placeholder={t("Complete Building A rough-in and pass inspection")} 
                />
                ...
                {duration > 0 && (
                    <div className="text-sm text-slate-500 bg-slate-50 dark:bg-slate-800 rounded-lg px-4 py-2.5 flex items-center justify-between">
                        ...
                        <span className="font-bold text-slate-800 dark:text-white">
                            {duration} {duration !== 1 ? t('days') : t('day')} ({Math.round(duration / 7 * 10) / 10} {duration >= 14 ? t('weeks') : t('week')})
                        </span>
                    </div>
                )}
                ...
                {(sprintForm.status === 'Completed' || sprintForm.status === 'Cancelled') && (
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                            {t("Sprint Retrospective Notes")}
                        </label>
                        <textarea
                            value={sprintForm.retrospectiveNotes || ''}
                            onChange={e => setSprintForm({...sprintForm, retrospectiveNotes: e.target.value})}
                            placeholder={t("What went well? What could improve? What will we commit to changing?")}
                            rows={4}
                ...
                <div className="flex justify-end gap-2 pt-4 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <Button onClick={handleSave}>
                        {sprintForm.id ? t('Update Sprint') : t('Create Sprint')}
                    </Button>
                </div>
            </div>
        </Modal>
