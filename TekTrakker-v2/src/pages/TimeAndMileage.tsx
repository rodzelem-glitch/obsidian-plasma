        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-6 pb-20">
            {saveFeedback && (
                <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[1000] bg-green-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-2 animate-bounce font-bold">
                    <CheckCircle size={20}/> {t(saveFeedback)}
                </div>
            )}
            ...
            <header className="flex justify-between items-start">
                ...
                <div className="flex gap-2">
                    <Button onClick={handlePrintPersonalLogs} variant="secondary" className="w-auto flex items-center gap-2 text-xs font-bold border-slate-200">
                        <Printer size={16} /> {t("Print Personal Logs")}
                    </Button>
            ...
            <Modal isOpen={!!viewingReceipt} onClose={() => setViewingReceipt(null)} title={t("Receipt Viewer")}>
                {viewingReceipt && <img src={viewingReceipt} alt="Receipt" className="w-full h-auto rounded" />}
                <div className="mt-4 flex justify-end">
                    <Button onClick={() => setViewingReceipt(null)}>{t("Close")}</Button>
                </div>
            </Modal>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-4">
                    <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase">{t("Weekly Mileage")}</p>
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400">{t("Total Recorded")}</p>
                </Card>
                <Card className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-4">
                    <p className="text-xs font-bold text-green-600 dark:text-blue-400 uppercase">{t("Expense Total")}</p>
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400">{t("Current Period")}</p>
                </Card>
            </div>
            ...
            <Card id="log-form-container" className={`transition-all duration-300 ${isEditMode ? 'ring-2 ring-amber-500 dark:ring-amber-400 bg-amber-50/10 dark:bg-amber-900/10 shadow-lg shadow-amber-500/20' : ''}`}>
                ...
                <form onSubmit={handleVehicleLogSubmit} className="space-y-4 mb-6">
                    <div className="flex flex-wrap gap-2">
                        <Button type="button" onClick={() => setNewVehicleLog({...newVehicleLog, type: 'Mileage'})} variant={newVehicleLog.type === 'Mileage' ? 'primary' : 'secondary'} className="flex-1 flex items-center justify-center gap-2"><MapPinIcon className="w-5 h-5"/>{t("Mileage")}</Button>
                        <Button type="button" onClick={() => setNewVehicleLog({...newVehicleLog, type: 'Fuel'})} variant={newVehicleLog.type === 'Fuel' ? 'primary' : 'secondary'} className="flex-1 flex items-center justify-center gap-2"><FuelIcon className="w-5 h-5"/>{t("Fuel")}</Button>
                        <Button type="button" onClick={() => setNewVehicleLog({...newVehicleLog, type: 'Maintenance'})} variant={newVehicleLog.type === 'Maintenance' ? 'primary' : 'secondary'} className="flex-1 flex items-center justify-center gap-2"><WrenchScrewdriverIcon className="w-5 h-5"/>{t("Maint.")}</Button>
                    </div>

                    <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-200 dark:border-gray-600">
                        <Toggle 
                            label={isCompanyVehicle ? t("Vehicle: Company Truck") : t("Vehicle: Personal Car")} 
                    ...
                    {newVehicleLog.type === 'Mileage' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="col-span-1 md:col-span-2">
                                <Input 
                                    label={t("Total Miles (if Odometer unknown)")} 
                                    type="number" 
                                    step="0.1" 
                    ...
                    {newVehicleLog.type !== 'Mileage' && (
                        <div className="space-y-4">
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">
                                {isEditMode ? t('Replace Receipt') : t('Upload Receipt')}
                            </label>
                            <div className="flex gap-2">
                                <Button 
                                    type="button" 
                                ...
                                >
                                    <CameraIcon size={16} /> {t("Take Photo")}
                                </Button>
                            ...
                            {(receiptFile || capturedReceiptData) && (
                                <p className="text-xs text-green-600 font-medium">✓ {t("Receipt ready")}: {receiptFile ? receiptFile.name : t('Photo Captured')}</p>
                            )}
                        </div>
                    )}
                    <Button type="submit" disabled={uploading}>{uploading ? t('Processing...') : (isEditMode ? t('Update Log') : t('Save Log'))}</Button>
                </form>

                <div className="space-y-3 max-h-96 overflow-y-auto custom-scrollbar">
                    <h4 className="text-md font-semibold text-gray-700 dark:text-gray-300 border-t border-gray-200 dark:border-gray-700 pt-4">{t("Recent Entries")}</h4>
                        ...
                        <div key={log.id} className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg flex justify-between items-center border border-gray-200 dark:border-gray-700">
                           <div>
                             <div className="flex items-center gap-2">
                                <p className="font-semibold text-gray-800 dark:text-gray-200">{t(log.type)}: {log.notes}</p>
                             ...
                             <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-1">
                                {log.date}
                                {log.startLocation && (
                                    <a href={`https://maps.google.com/?q=${log.startLocation.lat},${log.startLocation.lng}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-blue-600 hover:underline ml-2" title={`${t("Start time")}: ${log.startTime ? new Date(log.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : t('Unknown')}`}>
                                        <MapPinIcon className="w-3 h-3"/> {t("Start")}
                                    </a>
                                )}
                                {log.endLocation && (
                                    <a href={`https://maps.google.com/?q=${log.endLocation.lat},${log.endLocation.lng}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-emerald-600 hover:underline ml-2" title={`${t("End time")}: ${log.endTime ? new Date(log.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : t('Unknown')}`}>
                                        <MapPinIcon className="w-3 h-3"/> {t("End")}
                                    </a>
                                )}
                                {(!log.startLocation && !log.endLocation && log.location) && (
                                    <a href={`https://maps.google.com/?q=${log.location.lat},${log.location.lng}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-gray-500 hover:underline ml-2">
                                        <MapPinIcon className="w-3 h-3"/> {t("Location")}
                                    </a>
                                )}
                             </p>
                           </div>
                                        ...
                                        <tr className={latestEdit ? "bg-amber-50/30 dark:bg-amber-900/10" : ""}>
                                                ...
                                                {latestEdit && (
                                                    <div className="mt-1 flex flex-col">
                                                        <span className="inline-flex items-center w-max max-w-full px-2 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/60 dark:text-amber-200 shadow-sm border border-amber-200 dark:border-amber-700/50 break-words whitespace-normal" title={`Corrected by: ${latestEdit.adminName || 'Admin'}`}>
                                                            <span className="font-bold mr-1">{t("Edited:")}</span> {latestEdit.reason || t('Admin adjustment')}
                                                        </span>
                                                    </div>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex gap-2">
                                                    ...
                                                    ) : <span className="text-xs text-gray-400">{t("No Location")}</span>}
                                                    {log.endLocation && (
                                        ...
                                        {latestEdit && (
                                            <tr className="bg-amber-50/50 dark:bg-amber-900/20">
                                                <td colSpan={5} className="px-6 py-2 text-xs text-amber-800 dark:text-amber-200/80 border-t border-amber-100 dark:border-amber-900/50">
                                                    <strong>{t("Correction by")} {latestEdit.adminName || 'Admin'}:</strong> {latestEdit.reason}
                                                </td>
                                            </tr>
                                        )}
                                    </React.Fragment>
            ...
            {viewingReceipt && (
                <Modal isOpen={!!viewingReceipt} onClose={() => setViewingReceipt(null)} title={t("Receipt Viewer")} size="xl">
            ...
            {printHtmlContent && (
                <Modal isOpen={!!printHtmlContent} onClose={() => setPrintHtmlContent(null)} title={t("Print Preview")} size="xl">
                    <div className="flex flex-col h-[75vh]">
                        ...
                        <div className="mt-4 flex justify-end gap-3">
                            <Button variant="secondary" onClick={() => setPrintHtmlContent(null)} className="w-auto">{t("Cancel")}</Button>
                            <Button onClick={() => {
                                ...
                                <Printer size={18} className="mr-2" /> {t("Print Document")}
                            </Button>
                        </div>
                    </div>
                </Modal>
