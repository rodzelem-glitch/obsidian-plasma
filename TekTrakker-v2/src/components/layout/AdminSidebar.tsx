  ...
  return (
    <>
      ...
      >
        ...
        <nav data-tour="sidebar" className="flex-1 overflow-y-auto overflow-x-hidden overscroll-contain py-4 px-2 space-y-1 custom-scrollbar">
            {processedItems.map((item, idx) => {
                if (hiddenPaths.includes(item.path)) return null;
                
                if (item.isGroup) {
                    // Payments Only plan: hide group dividers to keep the restricted sidebar clean
                    if (isPaymentsOnly) return null;
                    return (
