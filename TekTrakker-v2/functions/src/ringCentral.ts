        ...
        if (event && event.body && event.body.parties) {
            ...
            if (party && party.from && party.from.phoneNumber) {
                ...
                if (!orgsSnapshot.empty) {
                    ...
                    adminsSnap.forEach((adminDoc: any) => {
                ...
                // Handle Missed Call
                let fromNumber = party.from?.phoneNumber;
                if (!fromNumber) return; // Can't SMS without a number
                            ...
                            try {
                                ...
                                const tokenResponse = await fetch(`${rcUrl}/restapi/oauth/token`, {
                                    ...
                                    headers: {
                                        ...
                                        'Authorization': `Basic ${Buffer.from(`${config.rcBackendClientId || config.ringCentralClientId}:${config.ringCentralClientSecret || ''}`).toString('base64')}`
                                    },
