 ...
 */
async function findCustomerByPhone(orgId: string, rawPhone: string) {
...
export const ringCentralWebhook = functions.https.onRequest(async (req, res) => {
    try {
        ...
        if (event && event.body && event.body.parties) {
            const party = event.body.parties.find((p: any) => p.direction === 'Inbound' && p.status?.code === 'Setup');
            if (party && party.from && party.from.phoneNumber) {
                let fromNumber = party.from.phoneNumber;
                if (fromNumber.startsWith('+1')) fromNumber = fromNumber.substring(2); // Normalize slightly
                ...
                if (!orgsSnapshot.empty) {
                    ...
                    let targetUserId: string | null = ownerId;
                    const toNumber = party.to?.phoneNumber;
                    if (toNumber && config?.rcMappings) {
                        let normTo = toNumber;
                        if (normTo.startsWith('+1')) normTo = normTo.substring(2);
                        const match = config.rcMappings.find((m: any) => {
                            let mNum = m.phoneNumber || '';
                            if (mNum.startsWith('+1')) mNum = mNum.substring(2);
                            return mNum === normTo;
                        });
                        if (match) {
                    ...
                    orgUsersSnap.forEach((userDoc: any) => {
                ...
                if (!orgsSnapshot.empty) {
                    ...
                    if (normFrom.startsWith('+1')) normFrom = normFrom.substring(2);
                            ...
                            try {
                                ...
                                if (tokenResponse.ok) {
                                    ...
                                    if (callLogResponse.ok) {
                                        const callLogData = await callLogResponse.json() as any;
        ...
        if (event && event.body && event.body.type === 'SMS' && event.body.direction === 'Inbound') {
            ...
            if (fromNumber && fromNumber.startsWith('+1')) fromNumber = fromNumber.substring(2);
...
export const registerRingCentralWebhook = functions.https.onCall(async (data, context) => {
...
export const ringCentralRecording = functions.https.onRequest(async (req, res) => {
    try {
...
export const fetchRingCentralCallLogs = functions.https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');

    const { orgId, phone } = data;
    if (!orgId) throw new functions.https.HttpsError('invalid-argument', 'Missing orgId.');
    ...
    try {
        ...
        if (!tokenResponse.ok) return { records: [] };
        const tokenData = await tokenResponse.json() as any;
        const accessToken = tokenData.access_token;
        ...
        if (!logRes.ok) return { records: [] };
        const logData = await logRes.json() as any;
        const records: any[] = [];

        (logData.records || []).forEach((r: any) => {
...
export const syncCustomerToRingCentral = functions.https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
    ...
    try {
        ...
        if (!tokenResponse.ok) return { success: false, reason: 'Failed to authenticate with RingCentral' };
        const tokenData = await tokenResponse.json() as any;
        const accessToken = tokenData.access_token;
        ...
        const existingContacts: any[] = contactsData.records || [];

        // Fetch customer or all customers
        let customersToSync: any[] = [];
        if (syncAll) {
...
export const onCustomerWriteSyncRingCentral = functions.firestore
    .document('customers/{customerId}')
    .onWrite(async (change, context) => {
        const afterData = change.after.exists ? change.after.data() : null;
        if (!afterData || !afterData.organizationId || !afterData.phone) return;
