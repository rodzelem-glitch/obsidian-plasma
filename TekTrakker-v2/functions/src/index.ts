...
import { trackSmsUsage } from './usageTracking';
...
export * from './aiAgent';
export { trackTelephonyUsage, trackSmsUsage, trackEmailUsage } from './usageTracking';
export * from './bidding';
...
let _billingBudgetsClient: BudgetServiceClient | null = null;
const getBillingBudgetsClient = () => {
    if (!_billingBudgetsClient) _billingBudgetsClient = new BudgetServiceClient();
...
let _billingClient: CloudBillingClient | null = null;
const getBillingClient = () => {
    if (!_billingClient) _billingClient = new CloudBillingClient();
...
export const GEMINI_PRO_MODEL = "gemini-3.8-flash";
...
export const handlePlatformInvoiceCommission = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        if (!existingCommDoc) {
            ...
            const effectiveRules: CommissionSettings = repData?.customCommissionSettings || rules;
            ...
            const isAnnual = items.some((item: any) => {
                const desc = (item.description || item.name || '').toLowerCase();
            ...
            const existingMonthMrr = repCommsSnap.docs.reduce((sum, doc) => {
                const data = doc.data();
                if (data.customerPaymentStatus !== 'Paid' && data.status !== 'Paid') return sum;
        ...
        try {
            const snap = await admin.firestore().collection('aiUsage')
                .select(
                    'totalTokensUsed', 
                    'promptTokensUsed', 
                    'candidatesTokensUsed', 
                    'virtualWorkerTokensUsed', 
                    'virtualWorkerPromptTokensUsed', 
                    'virtualWorkerCandidatesTokensUsed'
                )
                .get();
            snap.forEach(doc => {
                const data = doc.data();
...
async function getApiUsageMetrics(projectId: string) {
    ...
    for (const metricType of metricsToFetch) {
        try {
            ...
            usageMetrics[metricType] = (timeSeries[0]?.points || []).reduce((sum: number, point: any) => sum + Number(point.value?.int64Value ?? point.value?.doubleValue ?? 0), 0);
        } catch {
...
export const sendSms = functions.firestore.document('messages/{msgId}').onCreate(async (snap) => {
    ...
    if (!msg || msg.type !== 'sms' || (!msg.receiverId && !msg.to) || msg.direction === 'inbound' || msg.senderPhone) return;
...
export const generateReviewResponse = functions.runWith({ secrets: ["GEMINI_API_KEY"] }).https.onCall(async (data, context) => {
...
export const callLandingChatbot = functions.runWith({ secrets: ["GEMINI_API_KEY"] }).https.onCall(async (data, context) => {
...
export const callGeminiAI = functions.runWith({ secrets: ["GEMINI_API_KEY"], timeoutSeconds: 540 }).https.onCall(async (data, context) => {
    if (!context.auth) {
...
export const generateMarketingEmail = functions.runWith({ secrets: ["GEMINI_API_KEY"], timeoutSeconds: 300 }).https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError("unauthenticated", "User must be logged in.");
    const orgId = context.auth.token.organizationId || (await db.collection('users').doc(context.auth.uid).get()).data()?.organizationId || 'unauthenticated';
    const apiKey = await getGeminiApiKey(orgId);
    const { prompt } = data;
    ...
    try {
        ...
        const promptTokens = response.usageMetadata?.promptTokenCount || 0;
        const candidatesTokens = response.usageMetadata?.candidatesTokenCount || 0;
        await trackAiUsage(orgId, 'Marketing Content Generator', GEMINI_FLASH_MODEL, tokens, promptTokens, candidatesTokens);
