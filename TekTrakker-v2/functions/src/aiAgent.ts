...
import {
    ...
    isUserSupervisor,
    isUserMasterAdmin,
    AGENT_MODELS_CASCADE
} from './ai/config';
import { getAgentToolbox } from './ai/toolbox';
import { executeToolCall } from './ai/toolRouter';

// Re-export background tasks, helper functions, and tools for external callers and index.ts
export { getGeminiApiKey, trackAiUsage } from './ai/config';
export {
    undoAiAction,
    cleanupOldAiLogs,
    analyzeReceiptWithAI,
    processBackgroundAITask,
    generateCommercialReferenceSheetHelper,
    reportWorkerFailureHelper,
    reportWorkerFailure
} from './ai/backgroundTasks';
...
export const aiAgentController = functions.runWith({ secrets: ["GEMINI_API_KEY"], timeoutSeconds: 540 }).https.onCall(async (data, context) => {
    ...
    try {
        ...
        const isMasterAdmin = isUserMasterAdmin(userData);
        const isAdmin = isUserAdmin(userData);
        const isSupervisor = isUserSupervisor(userData, isAdmin);
        ...
        if (aiUsageDoc.exists) {
            ...
            if (limitTokens > 0 && totalTokensUsed >= limitTokens) {
                throw new functions.https.HttpsError(
                    'resource-exhausted',
                    'Your organization has depleted its AI token bucket. Please contact your administrator to upgrade your quota.'
                );
            }
        }
        ...
        const activeSynthesizedTools: any[] = [];
        ...
        const agentToolbox = getAgentToolbox(customDeclarations);
        ...
        if (history && history.length > 20) {
            ...
            if (summary) {
                formattedHistory = [
                    ...
                    ...messagesToKeep.map(formatHistoryTurn)
                ];
            } else {
                formattedHistory = history.map(formatHistoryTurn);
            }
        } else {
            formattedHistory = history.map(formatHistoryTurn);
        }
        ...
        let activeModelName: string = AGENT_MODELS_CASCADE[0];
        let lastError: any = null;

        for (const candidateModelName of AGENT_MODELS_CASCADE) {
            try {
                ...
                result = await executeWithRetry(() => session.sendMessage(messageContent), 3, 1500);
        ...
        while (currentResponse.functionCalls() && currentResponse.functionCalls().length > 0 && loopCount < maxLoops) {
            ...
            try {
                functionResponse = await executeWithRetry(() => activeChatSession.sendMessage(functionResponsesPayload), 3, 1500);
            } catch (loopError: any) {
                const isTransientLoop = 
                    loopError.status === 429 || 
                ...
                if (isTransientLoop) {
                    ...
                    const fallbackCandidates = AGENT_MODELS_CASCADE.filter(m => m !== activeModelName);
                    ...
                    for (const fallbackName of fallbackCandidates) {
                        try {
                            ...
                            functionResponse = await executeWithRetry(() => fallbackSession.sendMessage(functionResponsesPayload), 2, 1500);
...
export const askVirtualWorker = functions.runWith({ secrets: ["GEMINI_API_KEY"], timeoutSeconds: 540 }).https.onCall(async (data, context) => {
    return (aiAgentController as any).run(data, context);
});
