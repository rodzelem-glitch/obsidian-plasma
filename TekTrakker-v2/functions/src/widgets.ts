  ...
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      if (rawData.taxExemptDataUrl) {
        ...
        if (url) rawData.taxExemptUrl = url;
      ...
      if (formType === 'booking') {
        ...
        const formatWidgetPhone = (phoneStr: string) => {
          if (!phoneStr) return '';
          const cleaned = ('' + phoneStr).replace(/\D/g, '');
          if (cleaned.length === 10) return `(${cleaned.slice(0, 3)}) ${cleaned.slice(3, 6)}-${cleaned.slice(6)}`;
          if (cleaned.length === 11 && cleaned.startsWith('1')) return `(${cleaned.slice(1, 4)}) ${cleaned.slice(4, 7)}-${cleaned.slice(7)}`;
          return phoneStr;
        };

        const parseWidgetAddress = (fullAddr: string) => {
        ...
        if ((!city || !state || !zip) && streetAddress.includes(',')) {
          ...
          if (parsed.street && parsed.city) streetAddress = parsed.street;
        }
