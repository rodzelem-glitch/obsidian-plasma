...
const cleanUndefinedFields = (obj: any, seen = new WeakSet()): any => {
    ...
    try {
        ...
        if (!tilledUserId) {
            ...
            if (!createUserRes.ok) {
                ...
                const errData = await cloneRes.json() as any;
        ...
        if (!appRes.ok) {
            const errData = await appRes.json() as any;
            const errMessage = errData.message || '';
...
async function resolveJobIdFromFallback(
    db: any,
    eventData: any,
    allowedInvoiceStatuses: string[] = ['Unpaid', 'Partially Paid']
): Promise<string | null> {
    try {
        ...
        jobsSnapshot.forEach((doc: any) => {
            ...
            if (job.invoice && allowedInvoiceStatuses.includes(job.invoice.status)) {
                ...
                if (matchesTotal || matchesDeposit) {
                    ...
                    const nameWords = billingName.split(/\s+/).filter((w: string) => w.length > 2);
                    const jobWords = jobCustomerName.split(/\s+/).filter((w: string) => w.length > 2);
                    
                    const hasSharedWord = nameWords.some((w: string) => jobWords.includes(w));
    ...
    try {
        ...
        if (eventType === 'account.updated' || eventType === 'merchant.updated') {
            ...
            if (jobId && jobId !== 'unknown') {
                ...
                const processedIntents: string[] = Array.isArray(jobData.invoice?.processedPaymentIntentIds) 
                ...
                if (tipAmount > 0) {
                    const existingTipItem = updatedItems.find((it: any) =>
                        (it.paymentIntentId === eventData.id && it.type === 'Tip') ||
                ...
                if (processingFee > 0) {
                    const existingFeeItem = updatedItems.find((it: any) => 
                        it.paymentIntentId === eventData.id || 
                ...
                const updatedPayments = [...currentPayments.filter((p: any) => p.id !== eventData.id && p.paymentIntentId !== eventData.id), newPaymentRecord];
            ...
            if (jobId && jobId !== 'unknown') {
                ...
                const existingRefunds: any[] = Array.isArray(invData.refunds)
                    ? [...invData.refunds]
                ...
                if (webhookRefunds.length > 0) {
                    ...
                    const alreadyTracked = updatedRefunds.some(ex => 
                        ex.chargeId === eventData.id || 
 ...
 */
async function evaluateRecurringCommission(
    db: admin.firestore.Firestore,
    salesRepId: string,
    orgId: string,
    orgData: any,
    fixedSubscriptionTotal: number,
    today: Date
): Promise<{ shouldPay: boolean; commAmount: number; rateUsed: number; tier: string; notes: string } | null> {
    if (!salesRepId || fixedSubscriptionTotal <= 0) return null;

    try {
        const repDoc = await db.collection('users').doc(salesRepId).get();
        if (!repDoc.exists) return null;
        const repData: any = repDoc.data() || {};
        ...
        const globalRules: any = rulesDoc.exists ? rulesDoc.data() : {};
        const effectiveRules: CommissionSettings = repData.customCommissionSettings || globalRules;
                ...
                if (response.ok) {
                    ...
                    if (piData.status === 'succeeded' || piData.status === 'processing') {
                        ...
                        if (orgData.salesRepId && fixedSubscriptionTotal > 0) {
                            try {
                                const evalResult = await evaluateRecurringCommission(
                                    db,
                                    orgData.salesRepId,
                                    doc.id,
                                    orgData,
                                    fixedSubscriptionTotal,
                                    today
                                );
...
export const reconcileKortPayments = functions.runWith({ secrets: [kortSecretKey, kortAccountId] }).https.onCall(async (data, context) => {
    ...
    try {
        ...
        const piListResponse = await response.json() as any;
        const intents = piListResponse.data || [];
        const succeededIntents = intents.filter((pi: any) => pi.status === 'succeeded');
...
export const processCustomerMembershipBilling = functions.runWith({ secrets: [kortSecretKey, kortAccountId] }).pubsub.schedule('0 1 * * *').timeZone('America/New_York').onRun(async (context) => {
    const secretKey = kortSecretKey.value();
    const partnerAccountId = kortAccountId.value();
    if (!secretKey || !partnerAccountId) {
...
export const testCustomerMembershipBilling = functions.runWith({ secrets: [kortSecretKey, kortAccountId] }).https.onCall(async (data, _context) => {
    const secretKey = kortSecretKey.value();
    const partnerAccountId = kortAccountId.value();
    if (!secretKey || !partnerAccountId) {
    ...
    try {
        let docsToBill: admin.firestore.QueryDocumentSnapshot[] = [];
