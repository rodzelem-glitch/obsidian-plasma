...
exports.sendMonthlyCommercialStatements = exports.connectCallBridge = exports.initiateCallBridge = exports.outreachEmailWarmup = exports.autoTrackMileageOnClockOut = exports.userCalendarFeed = exports.checkProposalExpirations = exports.checkNewHireReporting = exports.checkApiKeyExpirations = exports.cleanupBidOnDelete = exports.automatedBidReminders = exports.postToX = exports.fetchShovelsPermits = exports.fetchIotDiagnostics = exports.enforceHardQuotas = exports.provisionCustomDomain = exports.automatedMaintenanceReminders = exports.twilioSubcontractorIVR = exports.twilioInboundSms = exports.twilioInboundVoice = exports.punchoutWebhook = exports.initiatePunchoutSession = exports.incomingLeadWebhook = exports.trackEmailOpen = exports.createUserAuth = exports.syncExternalReviews = exports.measureQuickWebhook = exports.sendgridWebhook = exports.processMailQueue = exports.manageHandshake = exports.suggestBidPricing = exports.generateBidDocument = exports.searchHistoricalBidData = exports.analyzeRFP = exports.callGeminiAI = exports.callLandingChatbot = exports.generateReviewResponse = exports.linkCustomerOnUserCreate = exports.deleteAuthUser = exports.sendSms = exports.getPlatformMetrics = exports.setUserRole = exports.checkUserInvite = exports.handlePlatformInvoiceCommission = void 0;
...
exports.handlePlatformInvoiceCommission = functions.firestore
    ...
    if (!existingCommDoc) {
        ...
        const subscriptionItem = after.invoice?.items?.find((item) => item.description && item.description.toLowerCase().includes('plan subscription')) || after.invoice?.items?.find((item) => item.description && item.description.toLowerCase().includes('platform subscription'));
        let planName = 'starter';
        if (subscriptionItem) {
            const desc = subscriptionItem.description.toLowerCase();
            if (desc.includes('growth'))
                planName = 'growth';
            else if (desc.includes('enterprise'))
                planName = 'enterprise';
            else if (desc.includes('payments_only') || desc.includes('payments only'))
                planName = 'payments_only';
        }
...
// --- MESSAGING ---
async function trackSmsUsage(orgId, direction) {
...
exports.sendSms = functions.firestore.document('messages/{msgId}').onCreate(async (snap) => {
    ...
    try {
        ...
        const response = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`, {
            method: 'POST',
            headers: {
                'Authorization': 'Basic ' + buffer_1.Buffer.from(`${accountSid}:${authToken}`).toString('base64'),
                'Content-Type': 'application/x-www-form-urlencoded'
            },
            body: body
        });
...
// --- AI USAGE TRACKING ---
async function trackAiUsage(orgId, taskName, modelName, tokenCount, promptTokenCount, candidatesTokenCount) {
...
exports.analyzeRFP = functions.runWith({ timeoutSeconds: 540, memory: '1GB' }).https.onCall(async (data, context) => {
    ...
    const apiKey = await (0, aiAgent_1.getGeminiApiKey)(orgId);
    let orgContext /* eslint-disable-line @typescript-eslint/no-explicit-any */ = null;
    if (orgId && orgId !== 'unauthenticated') {
        try {
    ...
    try {
        ...
        const analyses = await Promise.all(files.map(async (file /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
            ...
            if (fileData && (fileData.startsWith('http://') || fileData.startsWith('https://'))) {
                try {
                    const response = await axios_1.default.get(fileData, {
                    ...
                    const buf = buffer_1.Buffer.from(response.data);
                    ...
                    if (isXlsx) {
                        ...
                        fileData = buffer_1.Buffer.from(extractedText).toString('base64');
                        mimeType = 'text/plain';
                    }
                    else if (isDocx) {
                        const mammoth = await Promise.resolve().then(() => __importStar(require('mammoth')));
                        const result = await mammoth.extractRawText({ buffer: buf });
                        fileData = buffer_1.Buffer.from(result.value).toString('base64');
                        mimeType = 'text/plain';
                    }
                    else {
                ...
                catch (dlErr /* eslint-disable-line @typescript-eslint/no-explicit-any */) {
            ...
            try {
                ...
                if (err.message && (err.message.includes("503") || err.message.includes("429") || err.message.includes("overloaded"))) {
                    ...
                    catch (fallbackErr /* eslint-disable-line @typescript-eslint/no-explicit-any */) {
...
exports.generateBidDocument = functions.runWith({ timeoutSeconds: 540, memory: '1GB' }).https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (isGlobalEdit) {
            ...
            pricingTable += '--- | --- | --- | --- | --- | ---\n';
            let grandTotal = 0;
            lineItems.forEach((item /* eslint-disable-line @typescript-eslint/no-explicit-any */, idx) => {
            ...
            const questionsContext = (bid.questions || [])
                .filter((q /* eslint-disable-line @typescript-eslint/no-explicit-any */) => q.answer && q.answer.trim())
                .map((q /* eslint-disable-line @typescript-eslint/no-explicit-any */) => `Q: ${q.question || q.text || 'Unknown'}\nA: ${q.answer}`)
                .join('\n\n');
    ...
    try {
        ...
        if (!recentBidsSnap.empty) {
            historicalContext = recentBidsSnap.docs.map(doc => {
                const b = doc.data();
                return `Bid: ${b.title || 'Untitled'} (Status: ${b.status})\nAgency: ${b.agency || 'Unknown'}\nTotal Value: $${b.totalValue || 'Unknown'}\nItems: ${b.lineItems?.map((li /* eslint-disable-line @typescript-eslint/no-explicit-any */) => `${li.description} - qty ${li.qty} @ $${li.unitPrice}`).join(', ') || 'None'}`;
            }).join('\n\n');
        }
...
${JSON.stringify(bid.lineItems.map((item /* eslint-disable-line @typescript-eslint/no-explicit-any */) => ({
            id: item.id,
...
`;
        ...
        const updatedLineItems = bid.lineItems.map((item /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
            const rec = recommendations.find((r /* eslint-disable-line @typescript-eslint/no-explicit-any */) => r.id === item.id);
...
exports.manageHandshake = functions.https.onCall(async (data, context) => {
    ...
    if (action === 'approve') {
        ...
        const request = targetOrgData.partnerRequests?.find((r /* eslint-disable-line @typescript-eslint/no-explicit-any */) => r.fromOrgId === requestingOrgId);
        if (!request)
    ...
    if (action === 'reject') {
        ...
        const request = myOrgData?.partnerRequests?.find((r /* eslint-disable-line @typescript-eslint/no-explicit-any */) => r.fromOrgId === requestingOrgId);
        if (request) {
...
exports.processMailQueue = functions.runWith({ secrets: ["SENDGRID_API_KEY"] }).firestore.document('mail_queue/{docId}').onCreate(async (snap) => {
    ...
    try {
        ...
        if (smtpConfig) {
            try {
                ...
                const formattedSmtpAttachments = await Promise.all((payload.message?.attachments || []).map(async (att) => {
                    ...
                    if (!base64Data && urlOrPath && (typeof urlOrPath === 'string') && (urlOrPath.startsWith('http://') || urlOrPath.startsWith('https://'))) {
                        try {
                            const dlRes = await axios_1.default.get(urlOrPath, { responseType: 'arraybuffer' });
                            return {
                                filename: att.filename || att.fileName || 'attachment.pdf',
                                content: buffer_1.Buffer.from(dlRes.data),
        ...
        if (!sentViaCustomSmtp) {
            ...
            if (sendgridApiKey) {
                try {
                    ...
                    functions.logger.info(`Attachment check: hasAttachments=${!!rawAttachments}, count=${rawAttachments?.length || 0}, keys=${rawAttachments ? rawAttachments.map((a) => `[filename=${a.filename},contentLen=${(a.content || '').length},path=${a.path || a.url || 'NONE'}]`).join(', ') : 'N/A'}`);
                    if (payload.message?.attachments && payload.message.attachments.length > 0) {
                        msg.attachments = await Promise.all(payload.message.attachments.map(async (att) => {
                            let base64Data = att.content || '';
                            const urlOrPath = att.path || att.url || '';
                            if (!base64Data && urlOrPath && (typeof urlOrPath === 'string') && (urlOrPath.startsWith('http://') || urlOrPath.startsWith('https://'))) {
                                try {
                                    const dlRes = await axios_1.default.get(urlOrPath, { responseType: 'arraybuffer' });
                                    const dlBuffer = buffer_1.Buffer.from(dlRes.data);
...
exports.incomingLeadWebhook = functions.runWith({
    ...
    try {
        ...
        if (req.body.user_column_data && Array.isArray(req.body.user_column_data)) {
            req.body.user_column_data.forEach((col /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
        ...
        let customerId = '';
        let existingCustomerData /* eslint-disable-line @typescript-eslint/no-explicit-any */ = null;
        ...
        if (existingDoc) {
            ...
            const updates /* eslint-disable-line @typescript-eslint/no-explicit-any */ = {};
...
exports.punchoutWebhook = functions.https.onRequest(async (req, res) => {
    ...
    const rawXml = req /* eslint-disable-line @typescript-eslint/no-explicit-any */.rawBody ? req /* eslint-disable-line @typescript-eslint/no-explicit-any */.rawBody.toString() : req.body;
    ...
    try {
        ...
        const itemDescriptions = itemsField.map((i /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
...
exports.twilioInboundVoice = functions.https.onRequest(async (req, res) => {
    try {
        let resolvedOrgId = req.query.orgId;
        ...
        const initialGreeting = req.query.initialGreeting;
...
exports.twilioInboundSms = functions.https.onRequest(async (req, res) => {
    try {
...
exports.twilioSubcontractorIVR = functions.https.onRequest(async (req, res) => {
    try {
        const step = req.query.step || 'default';
        ...
        if (step === 'default') {
            ...
            if (jobDoc.exists) {
                ...
                const totalMins = timeEntries.reduce((acc, entry) => acc + (entry.timeOnSiteMinutes || 0), 0);
                const updates = {
                    checkOutTime: now,
...
exports.automatedMaintenanceReminders = functions.pubsub.schedule('0 9 * * *')
    ...
    try {
        ...
        for (const doc of customersSnap.docs) {
            ...
            customer.equipment.forEach((asset /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
...
__exportStar(require("./aiAgent"), exports);
__exportStar(require("./weeklyReferenceAudit"), exports);
exports.provisionCustomDomain = functions.https.onCall(async (data, context /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
    if (!context.auth)
    ...
    try {
        ...
        let fbConfig /* eslint-disable-line @typescript-eslint/no-explicit-any */ = {};
        try {
        ...
        const result = (await response.json()); // eslint-disable-line @typescript-eslint/no-explicit-any
        ...
        let dnsRecords /* eslint-disable-line @typescript-eslint/no-explicit-any */ = {};
...
exports.enforceHardQuotas = functions.pubsub.schedule('every 24 hours').onRun(async () => {
...
exports.fetchIotDiagnostics = functions.https.onCall(async (data, context) => {
    ...
    if (seamApiKey) {
        try {
            ...
            if (resp.ok) {
                const results = await resp.json(); // eslint-disable-line @typescript-eslint/no-explicit-any
                (results.devices || []).forEach((d /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
...
exports.automatedBidReminders = functions.pubsub.schedule('0 8 * * *').timeZone('America/New_York').onRun(async () => {
    try {
        const now = new Date();
        ...
        for (const doc of bidsSnap.docs) {
            ...
            if (bid.dueDate) {
                ...
                if (!isNaN(parsed.getTime()))
                    upcomingEvents.push({ name: 'Final Proposal Due', date: parsed });
            }
            if (bid.importantDates && Array.isArray(bid.importantDates)) {
                bid.importantDates.forEach((d /* eslint-disable-line @typescript-eslint/no-explicit-any */) => {
                    const parsed = new Date(d.date);
                    if (!isNaN(parsed.getTime()))
                        upcomingEvents.push({ name: d.name, date: parsed });
                });
            }
...
exports.cleanupBidOnDelete = functions.firestore.document('bids/{bidId}').onDelete(async (snap, context) => {
...
exports.checkApiKeyExpirations = functions.pubsub.schedule('0 9 * * *').timeZone('America/New_York').onRun(async () => {
    try {
        const now = new Date();
        now.setHours(0, 0, 0, 0);
        ...
        const promises = [];
        const expiringKeys = [];
        Object.entries(keysData).forEach(([serviceName, data]) => {
            if (data.expiresAt) {
...
exports.checkNewHireReporting = functions.pubsub.schedule('0 9 * * *').timeZone('America/New_York').onRun(async () => {
    try {
        const now = new Date();
        now.setHours(0, 0, 0, 0);
...
exports.checkProposalExpirations = functions.pubsub.schedule('0 9 * * *').timeZone('America/New_York').onRun(async () => {
    try {
        const now = new Date();
...
exports.userCalendarFeed = functions.https.onRequest(async (req, res) => {
    // Enable CORS
...
exports.autoTrackMileageOnClockOut = functions.firestore
...
exports.outreachEmailWarmup = functions.pubsub.schedule('0 9,14,18 * * *').timeZone('America/New_York').onRun(async () => {
...
exports.initiateCallBridge = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
    }
...
exports.connectCallBridge = functions.https.onRequest(async (req, res) => {
    try {
...
exports.sendMonthlyCommercialStatements = functions.pubsub.schedule('0 9 28-31 * *')
    .timeZone('America/New_York')
    .onRun(async () => {
    try {
